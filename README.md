...
- 팝업의 `응급 Undo` 버튼으로 마지막 삽입을 되돌릴 수 있습니다.
- 팝업의 `옵션` 드롭다운에서 각 기능을 개별적으로 켜고 끌 수 있습니다.
- 팝업에서 확장이 만드는 버튼 크기를 조절할 수 있습니다.
...
6. 문서가 망가졌다면 확장 팝업의 `응급 Undo` 버튼으로 마지막 삽입을 되돌릴 수 있습니다.

## 옵션 토글

확장 팝업의 `옵션` 드롭다운을 열면 아래 기능을 각각 켜고 끌 수 있습니다.

- `기능 활성화`
  - 복사와 붙여넣기 기능 전체를 한 번에 켜고 끄는 상위 토글
  - 꺼져 있으면 아래 옵션들도 자동으로 비활성화됩니다.
- `복사 버튼`
  - ChatGPT/Gemini 답변 옆의 복사 버튼 표시 여부
- `Paste to Notion 버튼`
  - Notion 우하단의 수동 붙여넣기 버튼 표시 여부
- `페이지 버튼 크기`
  - ChatGPT/Gemini/Notion 페이지 위에 표시되는 확장 버튼 크기를 `50% ~ 150%` 범위에서 조절

## 응급 Undo

- 팝업 안의 `응급 Undo` 버튼은 마지막 붙여넣기 1회를 되돌립니다.
- Notion API 특성상 블록을 하나씩 지우기 때문에 속도가 느릴 수 있습니다.
- 평소 작업용 기능이라기보다, 붙여넣기로 문서가 망가졌을 때만 사용하는 것을 권장합니다.
...
- `background.js`
  - 설정 저장, Notion API 호출, 요청 분할, 재시도, 취소 처리, 응급 Undo 처리
...
- 팝업의 `응급 Undo`는 확장이 마지막으로 추가한 top-level 블록을 지우는 방식이며, 다음 paste를 하면 이전 undo 대상은 덮어써집니다.
- `응급 Undo`는 여러 블록을 한 번에 지우는 API가 없어서 체감 속도가 느릴 수 있습니다.
